...
const CATEGORY_SECTIONS: {
  ...
  { category: "SHIRT", label: "Remeras", Icon: Shirt },
  { category: "PANTS", label: "Pantalones", Icon: SportShoe },
  { category: "SHOES", label: "Zapatillas", Icon: SportShoe },
  { category: "HAT", label: "Gorros", Icon: GraduationCap },
];
...
const ALL_TAB: { category: string; label: string; Icon: LucideIcon } = {
  category: "ALL",
  label: "Todos",
  Icon: LayoutGrid,
};
...
export default function StoreModal({
  ...
  const availableSections = CATEGORY_SECTIONS.filter((s) =>
    visibleItems.some((i) => i.category === s.category),
  );
              ...
              <div role="tablist" className="mb-4 flex gap-1.5">
                {tabs.map((tab) => {
                  ...
                  const count =
                    tab.category === "ALL"
                      ? visibleItems.length
                      : visibleItems.filter((i) => i.category === tab.category)
                          .length;
                  return (
                    <button
                      key={tab.category}
                      ...
                      className={`flex flex-1 min-w-0 items-center justify-center gap-1.5 rounded-xl px-2 py-2 text-[13px] font-semibold transition-colors ${
                        active
                ...
                {PURCHASE_FILTERS.map((filter) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-semibold transition-all active:scale-95 sm:text-[13px] ${
                        active
