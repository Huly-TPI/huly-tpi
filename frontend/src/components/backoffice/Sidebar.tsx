...
export default function Sidebar({ isOpen, onClose, onLogout }: SidebarProps) {
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 bottom-0 left-0 z-30 flex w-[304px] flex-col bg-white rounded-r-[32px]
      ...
      >
            ...
            {NAV_ITEMS.map(item => (
              <li key={item.to}>
                ...
                >
                  {({ isActive }) => (
                    <>
                      <span className={isActive ? 'text-white' : 'text-[#A0AEC0] group-hover:text-[#8869AC] transition-colors'}>
                        {item.icon}
                      </span>
                      {item.label}
                    </>
        ...
        <div className="shrink-0 px-4 pb-5 pt-2">
          ...
          <button
            onClick={onLogout}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-anaranjado/10 px-4 py-3 text-sm font-semibold text-anaranjado transition-all hover:bg-anaranjado/20 active:scale-[0.98]"
          >
