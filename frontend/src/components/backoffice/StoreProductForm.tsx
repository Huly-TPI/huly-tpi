...
const labelClass =
  "flex flex-col gap-1 text-sm font-semibold text-[#4A5568] dark:text-gray-300";
...
export function StoreProductForm({ product, onClose, onSubmit }: Props) {
  ...
  const canSubmit = Boolean(
    name.trim() && description.trim() && (isEdit || (imageLight && imageDark)),
  );
    ...
    <div className="fixed inset-0 z-[400] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      ...
      <div className="relative z-10 flex max-h-[85dvh] w-full max-w-lg flex-col gap-3 overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl dark:bg-[#172033]">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-[#8869AC]">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Cerrar"
            className="rounded-full p-1.5 text-[#A0AEC0] hover:bg-gray-100 dark:hover:bg-gray-800"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <label className="flex items-center gap-2 text-sm font-semibold text-[#4A5568] dark:text-gray-300">
          <input
            ...
            className="h-4 w-4 accent-violeta"
          />
          Solo premium
        </label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className={labelClass}>
            ...
            <input
              type="file"
              accept="image/*"
              onChange={(e) => setImageLight(e.target.files?.[0] ?? null)}
              className="text-xs text-[#4A5568] dark:text-gray-400 file:mr-2 file:rounded-lg file:border-0 file:bg-[#D1CAEF]/40 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-violeta"
            />
          </label>
          <label className={labelClass}>
            Imagen oscura
            <input
              type="file"
              accept="image/*"
              onChange={(e) => setImageDark(e.target.files?.[0] ?? null)}
              className="text-xs text-[#4A5568] dark:text-gray-400 file:mr-2 file:rounded-lg file:border-0 file:bg-[#D1CAEF]/40 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-violeta"
            />
          </label>
        </div>
