...
export function Toast({ message, onClose, duration = 6000 }: ToastProps) {
  ...
  return createPortal(
    <div className="fixed bottom-5 right-5 z-[9999] flex w-80 items-start gap-3 rounded-2xl bg-white px-4 py-3.5 shadow-xl ring-1 ring-[#D1CAEF]">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-[#8869AC]">Error</p>
        <p className="mt-0.5 text-xs text-[#4A5568] break-words">{message}</p>
      </div>
      <button
        onClick={onClose}
        className="shrink-0 mt-0.5 text-[#A0AEC0] hover:text-[#8869AC] transition-colors"
        aria-label="Cerrar"
      >
