...
export default function ProductsPage() {
  const { products, loading, error, clearError, create, update, remove } =
    useStoreProducts();
  ...
  const openEdit = (p: StoreItemResponse) => {
    setEditing(p);
    setShowForm(true);
  };
  const closeForm = () => {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <h1 className="text-[30px] font-extrabold leading-tight text-[#8869AC]">
            Productos de la tienda
          </h1>
          <p className="text-[16px] text-[#A0AEC0]">
            Alta, edición y baja de los cosméticos de la tienda
          </p>
        </div>
      ...
      {loading ? (
        <p className="py-8 text-center text-sm text-[#A0AEC0]">
          Cargando productos...
        </p>
      ) : products.length === 0 ? (
        <p className="py-8 text-center text-sm text-[#A0AEC0]">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((p) => (
            <div
              key={p.id}
              className="flex gap-3 rounded-2xl border border-[#EDF2ED] bg-white p-3 dark:border-gray-800 dark:bg-[#172033]"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 line-clamp-2 text-xs text-[#A0AEC0]">
                  {p.description}
                </p>
                <p className="mt-1 text-xs font-semibold text-[#4A5568] dark:text-gray-300">
                  {p.price != null
                    ? `$${p.price.toLocaleString("es-AR")}`
                    : `${p.priceCoins.toLocaleString("es-AR")} semillas`}
                  {p.premiumOnly && (
                    <span className="ml-1 text-violeta">· Premium</span>
                  )}
                </p>
                <div className="mt-2 flex gap-2">
                  <button
                    onClick={() => openEdit(p)}
                    className="flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-semibold text-violeta hover:bg-[#D1CAEF]/30"
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-semibold text-anaranjado hover:bg-anaranjado/10"
