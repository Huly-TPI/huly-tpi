...
public class PushNotificationSchedulerTest {
    ...
    @Test 
    void sendDailyNotifications_shouldSendPushToEachSubscription() throws Exception { 
        PushSubscription sub1 = PushSubscription.builder()
                                .id(1L).userId(10L).endpoint("https://fcm.example.com/1")
                                .p256dh("key1").auth("auth1").build(); 
        PushSubscription sub2 = PushSubscription.builder()
                                .id(2L).userId(20L).endpoint("https://fcm.example.com/2")
                                .p256dh("key2").auth("auth2").build();
        when(pushSubscriptionRepository.findAll()).thenReturn(List.of(sub1, sub2));
        scheduler.sendDailyNotifications();
        verify(pushNotificationAdapter, times(2)).send(any(), any());
    }

    @Test 
    void sendDailyNotifications_shouldContinue_whenOnePushFails() throws Exception {
        PushSubscription sub1 = PushSubscription.builder()
                                .id(1L).userId(10L).endpoint("https://fcm.example.com/1")
                                .p256dh("key1").auth("auth1").build();
        PushSubscription sub2 = PushSubscription.builder()
    ...
    @Test 
    void sendDailyNotifications_shouldDoNothing_whenNoSubscriptions() throws Exception { 
