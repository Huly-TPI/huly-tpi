...
class PushNotificationControllerTest {
        ...
        void subscribe_shouldReturn201_whenSubscriptionIsSaved() throws Exception {
                ...
                verify(savePushSubscriptionUseCase).execute(
                                eq(new SavePushSubscriptionRequest(1L, "https://fcm.example.com/1", "key1",
                                                "auth123")));
        }
        ...
        void status_shouldReturnSuscribedTrue_whenUserHasSuscription() throws Exception {
                when(getPushSubscriptionStatusUseCase.execute(new GetPushSubscriptionStatusRequest(USER_ID)))
                                .thenReturn(new GetPushSubscriptionStatusResponse(true, 20));
        ...
        void status_shouldReturnSuscribedFalse_whenUserHasNotSuscription() throws Exception {
                when(getPushSubscriptionStatusUseCase.execute(new GetPushSubscriptionStatusRequest(USER_ID)))
                                .thenReturn(new GetPushSubscriptionStatusResponse(false, 20));
        ...
        @Test
        void updateHour_shouldReturn204() throws Exception {
                mockMvc.perform(put("/api/pushNotification/hour").contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(Map.of("hour", 20))))
                                .andExpect(status().isNoContent());

                verify(updateNotificationHourUseCase).execute(USER_ID, 20);
        }
