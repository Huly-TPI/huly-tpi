...
public class EmailPortJavaMailImpl implements EmailPort {
    ...
    public void sendInactivityReminder(String to, String displayName, int rewardCoins) {
        try {
            ...
            helper.setText(buildInactivityHtml(displayName, rewardCoins), true);
            helper.addInline("logo", new ClassPathResource("email/logo.png"));
            mailSender.send(message);
            log.info("[MAIL] Email de inactividad enviado → to={} reward={}", to, rewardCoins);
        } catch (Exception e) {
            log.error("[MAIL] Error enviando email de inactividad a {} — {}", to, e.getMessage());
        }
    }

    private String buildInactivityHtml(String displayName, int rewardCoins) {
        return """
                <!DOCTYPE html>
                <html lang="es">
                <head>
                          ...
                          <tr>
                            <td style="padding:40px 48px 32px;">
                              ...
                              <p style="margin:0 0 16px;font-size:16px;color:#4a4a6a;line-height:1.7;">
                                Hace unos días que no te vemos por <strong style="color:#6c47ff;">Huly</strong>
                                y queremos que vuelvas a cuidarte con nosotros.
                              </p>
                              <p style="margin:0 0 28px;font-size:16px;color:#4a4a6a;line-height:1.7;">
                                Como regalo de bienvenida de vuelta, al iniciar sesión te esperan
                                <strong style="color:#6c47ff;">%d monedas</strong> 🪙.
                              </p>
                ...
                """.formatted(displayName, rewardCoins);
    }
